<template>
    <router-link 
      :to="`/partido-en-vivo/${match.id}`"
      class="match-card block relative overflow-hidden h-64 w-full lg:w-1/3 2xl:w-[30%] rounded-xl shadow-lg transition-transform transform hover:scale-105
             bg-white dark:bg-bg-cards-dark text-black dark:text-white"
      v-if="match && match.team1 && match.team2"
    >
      <!-- Fondo de los equipos -->
      <div class="absolute inset-0 z-0 flex justify-center items-center opacity-40">
      ...
      <div class="default-content absolute inset-0 z-20 flex flex-col justify-between p-4 bg-gradient-to-t from-black/80 to-transparent">
        <div class="flex justify-between items-center">
          <span class="league text-xs uppercase text-gray-300">{{ match.league }}</span>
          <span class="time text-xs font-bold text-primary-color">{{ match.time }}</span>
        </div>
      </div>
    </router-link>
  </template>
